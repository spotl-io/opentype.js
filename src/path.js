...
function roundDecimal(float, places) {
  ...
  const roundedDecimalPart = +(
    Math.round(decimalPart + "e+" + places) +
    "e-" +
    places
  );
...
function optimizeCommands(commands) {
  ...
  for (let i = 0; i < commands.length; i += 1) {
    ...
    if (cmd.type === "M") {
      ...
      cmd.type === "L" &&
      (!nextCommand || nextCommand.command === "Z")
    ) {
      ...
      cmd.type === "L" &&
      previousCommand &&
      previousCommand.x === cmd.x &&
      previousCommand.y === cmd.y
    ) {
      subpath.pop();
    } else if (cmd.type === "Z") {
...
Path.prototype.fromSVG = function (pathData, options = {}) {
  if (
    ...
    pathData instanceof SVGPathElement
  ) {
  ...
  function applyCommand() {
    ...
    const relative =
      commandType !== "Z" && command.type.toUpperCase() !== command.type;
    ...
    const currentX = this.commands.length
      ? this.commands[this.commands.length - 1].x || 0
      : 0;
    const currentY = this.commands.length
      ? this.commands[this.commands.length - 1].y || 0
      : 0;
    ...
    switch (commandType) {
      ...
      case "Z":
        if (
          ...
          this.commands[this.commands.length - 1].type !== "Z"
        ) {
          this.close();
        }
        break;
  ...
  for (const i in this.commands) {
    ...
    for (const prop in cmd) {
      if (["x", "x1", "x2"].includes(prop)) {
        ...
        this.commands[i][prop] =
          options.y +
          (flipY ? flipYBase - cmd[prop] : cmd[prop]) * options.scale;
      }
    }
  }
